...
describe("L3 protocol coordinator", () => {
  test("keeps editor writes, viewer projection, duplicate replay and resume explicit", () => {
    ...
    coordinator.join({ capability: "editor", clientId: editor, identity });
    coordinator.join({ capability: "viewer", clientId: viewer, identity });
    ...
    const resumed = coordinator.resume({ causalContext: {}, clientId: viewer, identity });
  ...
  test("expires presence and closes all sessions on ACL revoke", () => {
    ...
    coordinator.join({ capability: "editor", clientId: editor, identity });
    coordinator.join({ capability: "viewer", clientId: viewer, identity });
    coordinator.updatePresence({ clientId: editor, cursor: { blockId: identity.documentId, offset: 1 }, identity, ttlMs: 1_000 });
    assert.equal(coordinator.presence(identity).length, 1);
    now += 1_001;
    assert.equal(coordinator.presence(identity).length, 0);
    coordinator.updatePresence({ clientId: editor, cursor: null, identity, ttlMs: 10_000 });

...
4. 现有根`Dockerfile`使用Node 22、Webpack、思源路径和上游维护者元数据，不能代表奇点云端制品；将其改名为明确的上游参考文件。企业制品由三个镜像组成：API镜像用Node 24构建并运行Nest产物，Worker镜像用Node 24运行Nest后台任务并携带固定Go Kernel/`restore-archive`工具与appearance资源，Web镜像用Node 24构建Vite静态产物并由无特权静态服务器提供SPA回退。三者由部署入口保持同源，L0不在容器内增加临时反向代理或Kernel Gateway替身。Worker本地healthcheck同时核对PID 1、Node 24、两个Kernel可执行路径和英文appearance资源；CI在只读、断网、禁止pull的运行容器中实际启动`workspace restore-archive --help`并解析`appearance/langs/en.json`，不能仅凭Dockerfile文本或容器`running`状态声称恢复制品可用。
...
source commit + upstream baseline
  ...
  -> immutable metadata + read-only/offline Worker artifact probe + healthy smoke
...
| 企业API/Worker/Web镜像可重复构建并可追溯 | build | 真实`docker build`、OCI revision/upstream标签检查及三个镜像Docker healthy smoke |
| Worker交付真实恢复工具与appearance | build + artifact integration | 同一metadata入口在只读断网镜像中核对Node 24、可执行文件、英文语言JSON并实际启动`workspace restore-archive --help` |
| API与Worker审计签名身份一致 | integration | 同一smoke生成一次HMAC密钥与key version，并显式注入两个进程；密钥不进入镜像或报告 |

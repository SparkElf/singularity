...
| SBOM包含源码与两个镜像包清单 | supply-chain integration | 固定Trivy版本生成三份原始CycloneDX JSON；补证写独立canonical SBOM并共同上传 |
| API镜像只包含根运行图可达npm包，Web镜像不包含npm包 | supply-chain integration | 在只读断网API镜像内递归解析运行图，与原始SBOM按name、version和PURL双向比较 |
...
| 可修复High/Critical漏洞阻断且空报告不能假绿 | supply-chain integration | Trivy扫描企业源码与两个真实镜像，要求非空Target/Type，再由统一策略处理三份JSON |

...
4. Kernel请求在Go入口以严格JSON解码。查询最多512个Unicode code point，请求体
   ...
   空间级搜索和图谱聚合普通内容库与请求开始时仍已打开、已解锁的加密内容库。
   Kernel在固定该成员快照前取得全部加密笔记本的响应读门，并持有到JSON写入完成；
   `LockBox`因此不能与在途明文响应交错，已锁定内容库不进入结果或计数。搜索按稳定
   内容库顺序轮询合并，图谱按相同顺序给每个内容库分配总节点/边预算，不能让一个
   大内容库挤掉其余内容库。
...
8. 五条文档路由由service-auth middleware解析内容头一次。Go handler校验请求目标
   ...
   目录notebook项的`supportsGraph`是React唯一能力来源，值只由Kernel的
   `!box.Encrypted`产生，不能复制到document DTO，也不能由`locked`或其他字段推断。
   React只有在当前Selection的空间、笔记本、文档均与面板目标一致且
   `supportsGraph=true`时才能打开或请求局部图谱；Kernel对加密笔记本文档图谱在任何
   树或SQL查询前再次fail-closed。锁定notebook仍返回`supportsGraph=false`，但不产生
   document Selection。
...
9. 文档raw envelope只由React `discovery-api`解析一次，取出`data`后使用公共payload
   ...
   只能展示、不能导航，边只连接本次已经投影的节点。tag拓扑ID固定为`tag:<label>`，
   显示和搜索仍使用原始`label`；即使label等于某个block ID也必须保留两个不同节点及
   正确边。不得把请求的当前文档身份复制给所有节点，也不保留旧`box/rootID/id`响应
   别名作为第二合同。

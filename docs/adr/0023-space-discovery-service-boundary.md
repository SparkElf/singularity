...
7. 空间级查询没有预先选择的内容身份，因此使用上述公共Nest入口。文档范围面板
   ...
   只新增四条`read + content identity`策略：`/api/search/fullTextSearchBlock`、
   `/api/ref/getBacklink2`、`/api/history/searchHistory`和
   `/api/graph/getLocalGraph`。

8. 四条文档路由由service-auth middleware解析内容头一次。Go handler校验请求目标
...
9. 文档raw envelope只由React `discovery-api`解析一次，取出`data`后使用公共payload
   ...
   只能展示、不能导航。不得把请求的当前文档身份复制给所有节点，也不保留旧
   `box/rootID/id`响应别名作为第二合同。
...
11. 永久证据按真实边界分层：Nest HTTP合同覆盖授权、service identity、CSRF、
   ...
   覆盖两个公共空间路由；Gateway HTTP合同覆盖四条文档read policy和完整内容头；

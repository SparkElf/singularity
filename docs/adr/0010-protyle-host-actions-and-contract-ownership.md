...
| 1.11.2 | 2026-07-15 | Codex | 固定首请求所有权并以反链直接owner回调替代空身份与父模型扫描 |
| 1.12.0 | 2026-07-19 | Codex | 固定同文档多实例来源句柄、文档事件扇出与标题canonical回流 |
| 1.13.0 | 2026-07-19 | Codex | 固定内部剪贴板来源身份、跨空间复制及剪切存在性查询合同 |
| 1.14.0 | 2026-07-19 | Codex | 固定面包屑与父文档导航目标的显式三段身份合同 |
| 1.14.1 | 2026-07-19 | Codex | 按上游真实消费者校正P5旧壳Adapter保留与企业闭包删除边界 |
...
11. 旧壳Host可以承担语义事件到旧布局参数的真实协议转换，但不得成为fallback；它仍由上游desktop/mobile客户端消费时保留，P5只删除企业Web中的旧入口与错位owner，并以AST保证该Host不进入企业生产闭包。
...
14. 旧`App.plugins`适配器负责把可取消EventBus、用户快捷键、命令/斜杠注册和`preventDefault + resolve`转换为能力端口；这是协议与生命周期边界。其行为证据归App标准runner，企业Web不直接导入该适配器；上游客户端仍消费期间P5不得物理删除生产源码。
...
19. 公共`ProtyleFactory`只创建`workspace + live + bound content + automatic initial load`主编辑器，接收`host`、Session、唯一`notebookId`、唯一`documentId`、宿主`readOnly`和`AbortSignal`。`notebookId`在构造时绑定内容库，`documentId`只映射一次为Core `IProtyleOptions.blockId`；两者正交且生命周期内不由响应改写。Core内部构造器才显式接受`surface: workspace | embedded`、`participation: live | detached`与`content.mode: bound | local-only`，并只为bound内容选择首请求所有权。
...
39. S3创建包含正式零插件PluginPort、Menu、Overlay及其他能力的无Core完整Runtime；B4是React Host、公共Factory和Core接入该Session的唯一批次，不重建Runtime。P3只验证真实DOM与编辑行为，P4只增加非空插件和复杂内容外围能力，P5运行全链E2E并删除企业旧入口、错位Adapter owner与重复runner；上游客户端入口仅从企业闭包排除。
...
44. Core内部创建合同只在bound分支以必填`initialLoad: automatic | owner`声明首请求所有权；公共Factory固定`automatic`，迁入Vite闭包后持有查询、卡片或反链语义的直接所有者使用`owner`，local-only不含该字段。两种bound模式都必须先绑定非空、非占位的真实`blockId + notebookId`：automatic由Core经当前Session Transport发一次请求，owner使Core发零次自动请求并由直接owner经同一Transport和AbortSignal发一次请求，唯一成功响应都进入既有`onGet`初始化路径；销毁、换目标、错误或迟到结果不得改走另一入口。不得用空`blockId`、action、DOM、旧`fetchPost`或响应回填阻止、补做或重试首请求。
45. embedded反链结果中，成功transaction的`delete`和`move`是改变当前反链成员集合的完整触发集，普通文本编辑只由子Core就地渲染。前两类操作完成后由直接`Backlink`父所有者在创建边界提供的局部`() => void`回调刷新；该回调不进入公共Core options、HostEvent、EventBus或共享能力，也不得扫描DOM、布局、Registry寻找父模型。父owner以自身绑定身份刷新，并在owner存活、代次和挂载状态任一失效后拒绝迟到回调。
46. `ProtyleEditorHostEvent`只表达Core内部语义；事件跨入Session Runtime时由当前实例的Host facade一次附加稳定`sourceEditorId`，形成`ProtyleHostDispatchEvent`。运行时错误没有编辑器来源。宿主对块DOM、菜单、属性、活动状态和全屏等实例动作必须按该句柄精确定位；标题、图标和关闭等文档动作按显式`notebookId + documentId`扇出，不得按注册顺序、active实例或同文档首个实例近似来源。
47. Kernel是文档标题的唯一canonical owner。`RenameDoc`同一次规范化结果同时用于持久化、HTTP成功响应和`rename`推送；HTTP结果只校正发起实例并受提交代次约束，推送负责工作台和其他实例同步。浏览器输入规则可提供单行、Unicode长度和斜杠全角化体验，但不能把请求值直接发布为最终标题，也不能在API层再次规范化。
48. 内部剪贴板以版本化`data-siyuan`载荷把BlockDOM与复制源`spaceId + notebookId + documentId`一次绑定，HTML注释解析器是外部剪贴板字节的唯一协议owner。插件paste链只变换内容小载荷；一旦返回的`siyuanHTML`变化，原来源身份立即失效。只有来源空间等于当前Session时，粘贴端才以来源`notebookId + documentId`经当前空间Transport调用按笔记本隔离的`checkBlocksExist`；来源块已删除才保留ID。跨空间、无版本载荷、身份与BlockDOM不匹配或外部HTML一律按复制生成新ID，不从当前笔记本、全局blocktree、DOM或首个响应推断来源。
49. Kernel生成的breadcrumb节点统一为`blockId + notebookId + documentId`目标，删除含义模糊的`id`字段；反链、嵌入块、编辑器breadcrumb、大纲与移动端只消费该目标。嵌套文档的父文档导航同样由Kernel根据权威文档层级返回完整目标，Protyle只保存当前实例的临时`parentDocument`，标题、快捷键与菜单直接发送该目标；不得从`protyle.path`、当前文档身份或DOM补齐父目标。当前文档构造身份仍只来自目录选择且不被内容响应改写，父目标是正交的一次性导航元数据，不进入Zustand。
...
- **按`notebookId + documentId`选择首个编辑器实例**：拒绝。同一文档可重复页签或分屏，注册顺序不能表示菜单、选区、块DOM或全屏动作的来源；实例句柄只在Host facade跨Session边界附加一次。
- **让Kernel按`documentId`遍历已解锁加密内容库**：拒绝。它破坏笔记本隔离、扩大查询成本并把缺失字段掩盖成运行时fallback。
- **继续用当前笔记本或全局`checkBlocksExist`判断剪切**：拒绝。复制源与粘贴目标可能属于不同笔记本或空间，目标身份与全局命中都不能代表来源块是否仍存在，并会把跨库复制误判为剪切。
- **从breadcrumb路径、`protyle.path`或当前文档身份拼出父目标**：拒绝。路径只属于Kernel存储模型，当前文档ID不能代表父文档；由Kernel一次返回完整目标后，所有消费者只需转交同一三个字段。
...
- **继续用空`blockId`或action推断延迟首载**：拒绝。它让bound身份和请求所有权在首次请求时仍不完整；只在bound union增加二值`initialLoad`即可直接选定唯一请求方。
- **为automatic与owner建立两个构造器或初始化adapter**：拒绝。会复制身份、取消和错误状态；两种请求所有权共享既有`onGet`结果入口更简单。
- **让反链子编辑器发送workspace刷新、EventBus或共享adapter**：拒绝。局部父子更新会污染工作台或增加单消费者中间层；直接父owner回调最短且有明确销毁边界。
...
- 上游Host、Plugin Adapter、`EmbeddedProtyleOwner`与`Panel.ts`继续由思源客户端拥有；企业Web只消费公共Factory/Core入口，不持有这些上游实现的测试副本或运行时加载边。
...
- React创建边界固定为`spaceId + notebookId + documentId + readOnly`并内部补`automatic`；`notebookId`与`documentId`在构造时一次绑定，内容响应不得回填或改写。bound owner首载与automatic共用唯一`onGet`结果入口；无真实身份的detached实例仅本地渲染，不取得内容能力或首载模式。

...
| 1.4.0 | 2026-07-14 | Codex | 固定显式浏览器Runtime、表面语义与P2传输前置关系 |
...
18. Protyle构造器只接受显式`ProtyleRuntime`，不接受完整`App`；`IProtyle`不保存`app`。Runtime由单空间Session拥有，组合Host、PluginPort、Registry、Transport、Menu与Overlay能力。
19. 编辑器创建者必须显式声明`surface: "workspace" | "embedded"`。工作台表面可请求活动、布局、全屏与面板动作；嵌入式表面不得根据DOM、旧layout model或Registry资格猜测工作台身份。
20. `fetchPost`和旧`layout/Model`的错误、刷新与重连链会重新引入旧壳，因此Transport是B4闭包收口的前置子批；先完成Runtime合同，再完成P2 HTTP/WS端口，最后关闭真实入口闭包门禁。
21. Menu和BlockPanel的DOM行为留在浏览器Core；单例菜单、浮层集合、销毁顺序和未完成请求由Session拥有。不得继续读取`window.siyuan.menus.menu`或`window.siyuan.blockPanels`作为事实源。
...
24. `spaceId`只来自企业路由并由Session保存；Runtime中的Transport已经绑定Session，不重复该字段。旧壳不得以workspace路径、随机应用ID或Kernel实例ID近似；旧壳Adapter只验证过渡行为，真实隔离由P2/P5企业Session和Gateway完成。
25. Transport同时拥有HTTP请求和编辑器订阅；Menu只暴露当前会话菜单，Overlay只增量登记/遍历会话浮层。三者均提供幂等`dispose`，且不暴露旧App、可变全局数组、正文或布局对象。
26. `readOnly`由Factory/Controller与宿主授权事实拥有，不进入插件可变options；权限或错误只能收紧为只读，网络恢复和插件不能解除。
27. B4以可控浏览器外部边界证明请求/订阅取消与错误分类；真实Gateway HTTP/WS、授权和空间路由证据属于P5，二者不得互相冒充。
...
- B4与P2不再严格串行：Runtime合同属于B4，Transport实现属于P2前置子批，真实闭包门禁通过后才算P1-B4结束。
- 旧Web壳只在公共包外装配Runtime适配器；所有Protyle创建点一次性切换，不保留接收`App`的构造器重载。
- 菜单和浮层保持编辑器DOM语义，但全局集合不再属于`window.siyuan`。

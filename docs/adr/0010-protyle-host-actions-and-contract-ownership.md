...
19. 公共`ProtyleFactory`只创建`workspace + live`主编辑器，接收`host`、Session、唯一`documentId`、宿主`readOnly`和`AbortSignal`。Core内部构造器才显式接受`surface: workspace | embedded`与`participation: live | detached`，并使用`IProtyleOptions.blockId`作为唯一Core身份；公共`documentId`只映射一次。
...
29. `surface`只决定工作台与嵌入式宿主行为；`participation`是仅含`live | detached`的标量，只决定Registry和订阅。`live`从唯一Core `blockId`取得真实身份并在目标变化时重建；`detached`只有真实属性上下文才保留`blockId`，历史、快照和生成预览不得填空串、占位或近似ID。
30. 当前15个旧构造点不机械注入Session：主编辑器由React公共Factory替代，BlockPanel迁入Core，搜索、反链、卡片、历史、属性和资源预览随对应React所有者迁移，旧所有者退出生产闭包；`app/src/mobile/editor.ts`继续排除，不得为兼容旧入口放宽allowlist。
...
35. P3/P4 browser integration与P5 E2E使用互斥`testDir`和独立Playwright config；当前静态壳进入第三个独立目录。只有一个消费者的fixture和page object在B4内联删除，至少两个稳定合同出现后才可重新抽取技术support。

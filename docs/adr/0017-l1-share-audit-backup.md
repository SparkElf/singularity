...
9. Kernel跨进程响应由`ShareKernelClient`在字节解析边界核对返回`documentId`与请求中的显式身份，不匹配按服务不可用处理；核对后公开JSON只投影`assets`、`html`和`title`，不返回`documentId`、笔记本、空间或组织身份，控制面也不从响应或首个实例推断这些身份。
...
3. PostgreSQL自有的事件写入、组织序号推进和业务状态变更在同一数据库事务内完成。跨Kernel内容操作不宣称跨存储同事务：Gateway先持久化最小`content_audit_intents`，再调用Kernel，由声明式Worker按[ADR-027](0027-cross-kernel-content-audit-durability.md)追加明确结果或`indeterminate`。规范化载荷固定包含组织、可选空间、操作者、动作、目标、结果、发生时间和`requestId`，不包含令牌、密码、正文或任意大payload。
...
5. [ADR-027：跨Kernel内容操作审计耐久性](./0027-cross-kernel-content-audit-durability.md)
6. [奇点完整方案](../../output/md/Singularity_Enterprise_Knowledge_Base_v1.0.0_2026-07-13.md)

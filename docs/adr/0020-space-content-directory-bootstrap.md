...
| 1.3.0 | 2026-07-19 | Codex | 固定ContentSelectionScope代次能力与Session组合根的唯一选择写入合同 |
...
3. 笔记本项只包含`notebookId`、名称、图标、`locked`和`supportsGraph`。`supportsGraph`只由Kernel笔记本事实产生，是下游文档图谱能力的唯一来源；它不复制到文档项，也不从`locked`、名称、ID或其他状态推断。关闭笔记本不返回；锁定加密笔记本仍返回`supportsGraph: false`的自身壳，但不返回或暗示内部文档标识、标题、层级和数量，也不能产生document selection。文档项包含`notebookId`、`documentId`、标题、图标和`hasChildren`，不返回`.sy`路径、Kernel路径、正文、摘要、块数组或内部地址。每页固定最多128项并返回唯一`nextOffset: number | null`；文档项重复携带自身`notebookId`，使其进入选择状态时不必与可能已经换代的查询参数拼接身份。
...
9. TanStack Query拥有笔记本与各层分页响应，查询键完整包含`organizationId + spaceId + notebookId + level + offset`，其中`level`是根层或真实`parentDocumentId`的判别状态，不作为内容身份，并把AbortSignal传到请求。Zustand只新增当前内容选择`spaceId + notebookId + documentId + supportsGraph`这一跨树与编辑器共享的最小客户端状态，其中`supportsGraph`必须从当前notebook项复制；目录数组不复制进store，Session和Protyle实例仍不进入Zustand。树展开状态留在当前空间树owner；`SpaceSessionRoot`为每个授权空间路由代次激活唯一`ContentSelectionScope`，并通过一个直接render-prop交付Scope、活动Session、当前选择和有代次的选择命令，不建立全局Session Context或第二个Session store。
...
11. 所有目录请求和选择都绑定空间路由Scope generation，显式刷新再增加目录请求generation。切空间、401、隐藏式404、权限失效或锁态变化先使旧Scope、旧查询和选择失去提交资格；迟到页和旧Host/Discovery回调不得更新树、恢复旧选择或创建编辑器。初次目录503或网络失败显示可重试故障且不创建Protyle；已加载树的展开失败保留现有节点并显示本次未完成，不把错误改写为空数组、不切换备用文档。目录可用性错误不解除只读、不自动重复内容写入。
...
- S3可先创建真实空间Session，但B4只有在目录产生完整内容身份后才挂载Core；空间Session与文档选择保持两个正交生命周期。唯一组合根以`ContentSelectionScope`把两者绑定到同一授权空间代次，目录、HostEvent和Discovery不能绕过该能力写入选择。

...
- `app/src`当前有15个`new Protyle(...)`创建点，全部属于旧壳或原生移动所有者。旧壳没有企业`spaceId`，因此这些点不能机械改为Session；主编辑器由React公共Factory替代，嵌入式所有者随对应功能迁移，旧所有者退出生产闭包。
...
```text
React workspace
  | authorized SpaceRuntimeBootstrap, documentId
  v
Space route composition root
  | one Session for the validated spaceId
  v
ProtyleHost.tsx
  | public Factory: create(host, session, documentId, readOnly, signal)
  v
...
现有Protyle在构造时通过`IProtyleOptions.blockId`绑定文档，没有公共`openDocument`合同。创建边界分为两层：React主编辑器只使用公共Factory，公共Factory固定创建`workspace + live`；搜索、反链、卡片、浮层、历史和资源预览等迁入Vite闭包的Core所有者使用内部构造合同，显式声明`surface`与`participation`。旧壳不创建生产Session，也不作为第三种创建边界。
...
interface ProtyleCoreBaseOptions {
  host: HTMLElement;
  session: ProtyleSession;
  readOnly: boolean;
  signal: AbortSignal;
...
type ProtyleCoreCreateOptions = ProtyleCoreBaseOptions & (
  | {
      participation: "live";
      options: IProtyleOptions & { blockId: string };
    }
  | {
      participation: "detached";
      options: Omit<IProtyleOptions, "blockId"> & { blockId?: string };
    }
);
...
- 公共Factory把唯一`documentId`一次映射为Core的`IProtyleOptions.blockId`，固定补入`surface: "workspace"`与`participation: "live"`；Core内部不再接收第二个`documentId`。
- 内部构造器接收`session`、`host`、`readOnly`、`signal`、唯一`surface`、标量`participation`和现有`IProtyleOptions`；它不得再接收Runtime、Host或文档身份副本。
- `live`要求`options.blockId`为真实身份并加入Registry与实时订阅；`detached`不加入二者，只有属性ghost等真实上下文才保留`blockId`，历史或生成预览不得填入空串、占位或近似ID。
- 公共Factory中的`documentId`或`session`变化时销毁旧实例并创建新实例；嵌入式live所有者切目标也必须重建。
- `readOnly`变化只调用`setHostReadOnly`，不修改Core从真实文档响应取得的属性只读，也不复制编辑器状态。
- `surface`与`participation`由直接创建合同显式确定；同一实例生命周期内不变化，也不从DOM、action或Registry资格互相推断。
- 异步创建使用`AbortSignal`和实例代次，迟到的控制器立即销毁，不能覆盖新文档。
- `destroy`必须幂等，并释放该实例拥有的订阅、DOM监听器、Observer、插件事件和动态资源引用；不得提前释放Session级Transport、Menu或Plugin能力。
...
| `refresh-outline` | `documentId` | 刷新当前文档大纲，不负责打开面板 |
| `refresh-backlinks` | `documentId` | 刷新当前文档反链，不负责打开面板 |
...
只有`surface: "workspace"`可以发送刷新面板、元数据、活动、全屏、布局和统计事件；`embedded`仍可发送导航、资源和通知等显式宿主动作，但不能借助DOM或Registry资格修改工作台状态。该规则由真实Core事件消费的unit证据和P3浏览器集成共同保护。
...
`ProtyleSession`只保存唯一`spaceId`、唯一`runtime`并暴露`retrySubmission()`与有序`dispose()`；Host不在Session上重复暴露，因为它已经是Runtime能力。Factory和Core内部构造器都只接收Session，不另收Runtime；Core只从`session.runtime`取得能力。生产唯一数据链为`SpaceRuntimeBootstrap -> ProtyleSession -> ProtyleRuntime -> Core`。

Core内部构造器另收唯一`surface`和标量`participation`。`workspace`表示React工作台拥有的主编辑表面，可以请求活动状态、面板同步、全屏和布局持久化；`embedded`表示搜索、反链、浮层、历史和属性预览等由局部所有者销毁的表面。`live`加入Registry和当前编辑器订阅，`detached`跳过二者；两字段互不推断，也不从DOM类名反推。所有进入Vite生产闭包的新所有者显式声明两字段，不保留接收`App`的重载。

当前15个旧构造点按下表处置，不向缺少企业身份的旧壳注入Session：

| 旧创建点 | 目标处置 | 新合同 | 完成条件 |
| --- | --- | --- | --- |
| `editor/index.ts` | React主编辑器替代 | 公共Factory固定`workspace + live` | 真实空间与文档路由创建，旧页签所有者不进入闭包 |
...
| Factory | 公共层固定创建`workspace + live`，Core层承接显式内部组合 | 采用；不向React暴露内部生命周期组合，也不复制文档身份 |
...
| 公共Factory固定`workspace + live`，Core构造器显式声明表面与参与方式，文档身份只映射一次 | typecheck + unit + static | 真实Factory/Core类型、React主编辑器和迁入Vite闭包的嵌入式所有者；只模拟外部资源加载 | 扩展`ProtyleHost`组件测试与AST边界；旧壳和原生移动点显式排除 |
...
| `documentId`/Session变化重建，迟到实例销毁，宿主只读就地更新 | component | 真实React effect；只模拟外部Protyle Factory | 扩展现有`ProtyleHost.test.tsx`，不另建重复组件测试 |
...
1. **B4-Runtime合同**：落盘PRD，扩充canonical Session/Runtime、公共Factory、Core内部构造器、surface、participation、错误类别、重试和最小工作台事件；完成条件是公共Factory固定`workspace + live`，Session只持`spaceId + runtime`，Core只经Runtime取能力，participation不携带文档身份。当前公共包与React Host局部证据已具备。
...
- **对象字段唯一性**：公共Factory只接收`documentId`并一次映射为Core `blockId`；`participation`只表达生命周期，不再携带第二份身份。

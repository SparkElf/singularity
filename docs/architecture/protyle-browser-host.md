...
| `fetchPost`、上传XHR、`layout/Model` WebSocket | 暂留旧Core，未视为企业传输 | 全部改为Session Runtime的Transport/Upload/Subscribe，统一取消、错误和空间身份 |
...
边界不同时传`workspaceId`、`blockId`、`rootId`等同义或近似字段。`notebookId`只选择内容库，不是第二个文档ID；`documentId`到Kernel `blockId`的协议命名只在浏览器编辑器内部转换一次。目录按笔记本根或真实父文档每页128项返回最小项，不公开路径或正文；浏览器不能把路由、DOM、全局状态、首个内容响应或笔记本ID解释为缺失文档身份。
...
| HTTP/WS/FormData绑定spaceId与内容身份，上传携带内存CSRF并支持取消/进度，错误语义和销毁终止一致 | B4 unit + S2 integration + P4 browser integration + P5 E2E | B4只替换浏览器`fetch`/`WebSocket`/`XMLHttpRequest`外部边界；S2以真实HTTPS/WSS驱动Gateway并使用受控外部Kernel；P4驱动真实上传进度与只读拒绝 | 公共Transport原生case与S2真实Gateway合同同批删除旧fetch/Model/上传XHR、任意header字典、`uploadUrl`及`ModelReconnectLifecycle.test.js`，不保留双实现 |
...
- 当前实现状态：临时HTML Core已隔离并删除；真实入口骨架和企业侧显式失败语义已落盘，但真实入口尚未接入企业Vite。未完成项仍包括Runtime Transport/WS替换、插件/菜单/资源端口迁移、`IProtyle.app`及旧工作台依赖消除、真实入口闭包allowlist和P3浏览器行为证据。
- 完成条件：公共事件和插件合同类型通过；Protyle源码无旧工作台动作import、无`protyle.app`；浏览器入口闭包只含批准模块；static、unit、component与build集中通过。未达到这些条件前，企业Factory必须保持显式失败，不得添加HTML或旧App fallback。
...
- 完成条件：真实登录、选空间、打开/编辑/保存/重载、搜索、块引用、AV、插件、越权写拒绝、主动内容隔离和撤权清屏均通过；旧Web入口文件、旧Adapter和重复runner物理不存在；AST证明Vite是唯一Web生产闭包，并满足L1第9.3节对应的生产Protyle验收。L0第8.3节基础工程验收已独立完成。

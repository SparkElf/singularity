...
本方案把最小企业空间启动切片前移到Protyle生产迁移之前。目标是让生产数据链固定为`认证会话 -> 持久化Space -> 授权后的运行时启动响应 -> ProtyleSession -> Runtime -> Core`，并使浏览器永远不知道Kernel内部地址、工作空间路径或服务凭证。

本方案不提前实现完整成员管理、用户组、分享、审计UI、备份或实时协作。它只前移Protyle生产装配所必需的本地账号会话、组织与空间最小模型、空间角色、Kernel实例解析和HTTP/WebSocket Gateway。
...
| 证据 | 当前事实 | 架构影响 |
| --- | --- | --- |
| `enterprise/apps/web/src/app/App.tsx` | 只有固定`/workspace`路由 | 路由不能提供真实空间身份 |
| `enterprise/packages/protyle-browser/src/session.ts` | 只能消费调用方提供的`spaceId` | 公共包不是身份生产者 |
| `app/src/index.ts` | 旧入口无参数创建`App` | 旧壳没有可注入的企业空间上下文 |
| `kernel/conf/system.go` | `System.ID`是设备ID，`workspaceDir`是路径 | 两者不得作为空间主键 |
| `app/src/constants.ts` | `SIYUAN_APPID`随机生成 | 只表示旧连接实例，不是空间身份 |
| `kernel/model/sync.go` | `KernelID`和`cloudName`属于同步实现 | 不能组合或推断`spaceId` |
...
本轮使用仓库源码、既有ADR和权威方案完成审计，本地证据充分，未使用网页搜索。

### 2.2 已确认缺口

- 当前没有生产代码创建带真实`spaceId`的`ProtyleSession`。
- 当前没有NestJS API、Prisma模型、认证Cookie或空间启动接口。
- 当前没有将Kernel API路径分类为读取、写入和管理操作的权威策略清单。
- 当前公共包闭包尚未接入真实Core，绿色构建不能证明生产空间隔离。

## 3. 决策与范围

### 3.1 顺序决策

在恢复Protyle Core迁移前必须完成四项前置能力：
...
`apps/api`保持单副本模块化单体，不为启动切片拆微服务。`kernel-client`承担私网协议、短期服务凭证、生命周期、响应清洗和错误语义转换，不暴露到浏览器；`authorization`拥有`HTTP方法 + canonical route template`到授权动作的唯一策略清单，未知组合默认拒绝。L5横向扩展前必须先把连接撤权事件从进程内发布替换为跨副本消息总线，本期不同时维护两套传播路径。
...
- B4才在同一组合根中把Session、唯一`documentId`和由角色派生的宿主`readOnly`交给`ProtyleHost`与公共Factory。
- 在路由空间身份变化或组件卸载时按Session合同有序销毁。

Session是带资源生命周期的非序列化对象，由空间路由组件直接拥有，不进入Zustand。Zustand只保存跨页面且可序列化的工作台状态；TanStack Query保存服务端返回的组织、空间、角色和启动状态。
...
| 组织或空间变为不可见 | `404 + not-found` | 启动前不创建Session；活动Session中的后续请求映射为`forbidden`并销毁 |
...
`4408 + client-messages-forbidden`表示浏览器代码违反只接收推送的协议不变量，Transport按不可重试的`forbidden`终止连接并记录诊断。两套合同不增加同义`retryable`或`source`字段；Gateway通过状态、Problem code和受信上游结果决定唯一映射。

### 5.3 Kernel Gateway合同
...
Gateway按固定顺序执行认证会话、CSRF/Origin、组织成员、空间成员、Kernel路由策略和实例解析。数据库只保存可信部署句柄，不保存任意URL；部署句柄由受信运行时注册表解析为固定`https/wss`协议、服务证书身份和端口。浏览器提交的URL、Host、工作区路径、代理地址和实例ID一律不参与路由。
...
每个事实只有一个所有者。路由参数只定位查询，PostgreSQL拥有空间和角色，Kernel实例注册表拥有内部路由，Session拥有浏览器运行时资源，Kernel拥有内容。

### 6.2 状态与失效

- Gateway维护进程内`SpaceConnectionRegistry`，按`authSessionId`、`userId`、`organizationId`和`spaceId`四个索引登记浏览器WebSocket；连接状态只取`pending`、`active`或`closed`，Registry不保存正文或消息副本。本期部署合同明确限制单个API副本，L5横向扩展前必须以消息总线替换进程内撤权事件并通过跨副本合同测试。
...
- `AuthSessionService`撤销或到期、`UserService`禁用、`OrganizationService`停用、`OrganizationMembershipService`变更、`SpaceService`停用和`SpaceMembershipService`变更必须在数据库事务提交后发布唯一`AccessChanged`领域事件。会话自然到期由Registry按绝对/空闲期限定时关闭；HTTP活动延长空闲期限后，AuthSessionService发布同一会话的到期更新时间，Registry只重排该计时器。
...
- 生产会话Cookie固定为`__Host-singularity_session`，属性为`Secure; HttpOnly; Path=/; SameSite=Lax`且禁止`Domain`。登录成功和权限提升时轮换不透明令牌；数据库只保存令牌摘要、30分钟空闲到期和12小时绝对到期，时钟由认证服务统一消费。
- `GET /api/v1/auth/csrf`签发绑定当前AuthSession的CSRF token，浏览器只保存在内存。所有非安全HTTP方法同时校验`X-CSRF-Token`和精确允许的`Origin`；登录接口至少校验精确Origin并受账号/IP速率限制。WebSocket升级校验精确Origin和会话Cookie，不接受查询参数凭证。
- 本地密码使用Argon2id摘要，登录错误不区分账号不存在与密码错误；会话撤销和用户禁用按6.2节关闭现有连接。
- 所有组织与空间查询同时约束认证用户和资源所有权，不接受客户端声明的角色。
...
- Gateway到Kernel强制mTLS与短期服务JWT，主动内容按5.5节强制下载或安全渲染；应用源不执行上传或导出的HTML、JavaScript、SVG、XML和PDF。
...
- **Adapter**：`kernel-client`把思源HTTP/WS协议、内部认证和错误转换为企业Gateway语义，承担真实信任边界与生命周期差异。
...
- **Event-Driven**：事务提交后的唯一`AccessChanged`事件驱动连接关闭与到期更新，业务Service不直接操作WebSocket对象。

不新增浏览器全局Singleton、第二个Session store、DTO同形mapper或旧App兼容桥。网络边界只解析合同必要字段，不在编辑热路径重复校验或拷贝内容payload。
...
S0至S3固定由`enterprise/package.json`拥有唯一聚合命令名`pnpm verify:s0-s3`，但命令内容随S0、S1、S2、S3在同一能力批次原子扩展。不得在S0提前注册未来空suite，也不得使用`passWithNoTests`让缺失能力绿色；任何新package、Vitest project或Go package进入聚合命令时，必须在同一批至少带一个稳定合同case。
...
verify:s0-s3
  ...
  -> test:s0-s3
     ...
     -> @singularity/api test
        -> Vitest unit project
        -> Vitest http-contract project
        -> Vitest integration project
     -> @singularity/web test:space-session
     -> go -C ../kernel test -vet=off ./serviceauth
  -> build:s0-s3
     -> contracts/database/authorization/kernel-client production builds
...
| S1 | Auth/Space HTTP contract、会话时钟case、React组织/空间路由component | 预注册S2 Gateway或S3 Session空suite |
...
| S3 | React无Core Session组合根component和Web生产build | 用测试Factory、静态shell或空Playwright目录冒充 |

`lint:s0-s3`覆盖`apps/api`、Web空间组合根、contracts、database、authorization和kernel-client；`typecheck:s0-s3`调用这些package各自的`typecheck`；`test:s0-s3`只调用package声明且lockfile锁定的runner；`build:s0-s3`证明Node packages、Nest应用和Web可生成生产产物。现有`kernel-baseline`继续承担上游Kernel广泛回归，但S2服务认证package测试必须同时由`verify:s0-s3`直接发现。

`.github/workflows/singularity-l0.yml`从S0新增独立`space-session` job，使用`postgres:17-alpine` service，固定测试值`POSTGRES_USER=singularity_test`、`POSTGRES_PASSWORD=singularity_test`、`POSTGRES_DB=singularity_test`，并以`pg_isready -U singularity_test -d singularity_test`作为healthcheck。job注入`SINGULARITY_TEST_DATABASE_URL=postgresql://singularity_test:singularity_test@127.0.0.1:5432/singularity_test`，冻结安装`enterprise/pnpm-lock.yaml`并只调用唯一聚合命令。S2在同一job原子增加`actions/setup-go`、`kernel/go.mod`版本和临时测试CA/证书生成，再继续只调用`pnpm verify:s0-s3`；不得把Go服务认证留在另一个required check中。当前本地`127.0.0.1:5432`无响应，未经用户同意不得启动或重配；静态与unit可以执行，但数据库、真实HTTPS/WSS integration不得标记为通过。
...
Nest测试应用和受控Kernel HTTPS/WSS服务都由runner `beforeAll/afterAll`在随机端口启动与关闭；临时CA、服务端证书、客户端证书和JWT密钥由测试生命周期创建并在失败路径删除。固定sleep禁止用于同步，HTTPS等待响应，WebSocket等待open/message/close事件。

### 9.3 稳定合同矩阵

| 稳定合同 / 主要失败原因 | 最低层级 | 真实与模拟边界 | 独立证据 |
| --- | --- | --- | --- |
| Prisma迁移可从空schema回放 | database integration | 真实PostgreSQL | 独立migration case |
...
| Cookie轮换、绝对/空闲到期和CSRF/Origin拒绝 | HTTP contract | 真实Nest HTTP与数据库；时钟为外部替身 | 200、401和403独立case |
| 启动响应只含权威字段 | HTTP contract | 真实Nest HTTP、授权和数据库；Kernel状态端口为外部替身 | 200及三种kernelState独立case |
| 不存在或不可见空间统一不泄露 | HTTP contract | 真实HTTP与数据库 | `404 + not-found`独立case |
...
| `apps/api/test/support/test-app.ts` | runtime HTTPS合同、Gateway HTTPS/WSS | 随机端口启动/关闭Nest应用，返回基址 | 登录、授权或Gateway成功判断 | beforeAll/afterAll；runner原生能力可替代时删除 |
| `apps/api/test/support/kernel-server.ts` | Gateway策略/错误映射、资源主动内容、WS撤权 | 受控外部HTTPS/WSS、临时mTLS身份、原始请求与响应证据 | 预设授权结论和业务payload | beforeAll/afterAll；真实Kernel覆盖同一协议后缩减或删除 |

support不建立barrel或同形wrapper，消费者直接导入。业务fixture显式创建合同需要的最小字段，cleanup与创建在同一API暴露。

### 9.5 Playwright阶段门禁

当前`browser-integration`和`e2e`目录为空，`--pass-with-no-tests`命令只用于发现配置，不属于`verify:s0-s3`或生产门禁。第一个永久测试落地时必须在同一批删除对应`--pass-with-no-tests`，否则CI不得调用该命令。

Browser integration允许拦截外部Gateway，配置只启动Vite Web并保持`fullyParallel: true`，每个case自备route状态。P5真实E2E配置由Playwright `webServer`数组启动API、Web和测试Kernel，`globalSetup`执行专用测试schema迁移；在按worker创建并清理组织/空间的fixture具备两个稳定消费者前使用`fullyParallel: false`。现有静态shell测试继续独立，不能作为Session、Gateway或E2E证据。
...
1. 实现本地账号登录、同源Cookie、会话撤销和用户禁用。
2. 实现CSRF、精确Origin、token轮换、绝对/空闲到期和登录速率限制。
3. 实现组织、空间、两级成员查询及`SpaceRuntimeBootstrap`真实HTTP Problem合同。
4. 建立事务提交后的`AccessChanged`领域事件，覆盖会话撤销/到期、用户禁用、组织/空间停用及两级成员变化，不提前关闭连接或失效查询。
5. 实现企业React组织/空间路由，不再渲染硬编码“默认空间”。
6. 原子扩展`verify:s0-s3`加入非空HTTP contract与路由component suite。
7. 完成条件：浏览器只能从授权响应取得真实`spaceId`，401/403/404、CSRF、会话时效和全部AccessChanged生产者合同通过。
...
4. 实现空间化Kernel API、assets、upload、exports和只接收推送的Protyle WebSocket代理；主动内容按5.5节强制下载或安全渲染。
5. 实现pending/active单副本`SpaceConnectionRegistry`、四索引、授权复验、到期计时器、4401/4403/4408关闭语义及全部`AccessChanged`消费。
...
1. 实现生产HostPort、正式零插件PluginPort、Registry、绑定空间Gateway的Transport、ResourcePort、Menu、Overlay、完整Runtime和Session组合，不接入编辑器Core。
2. 仅在授权启动状态为`ready`时创建Session，切空间和撤权按“冻结命令、等待dispose、清DOM、失效查询、代次校验”顺序销毁。
3. 以非编辑器就绪状态证明完整Runtime和真实Session可由路由持有，不使用占位能力、测试Session、测试Factory或硬编码空间ID。
4. 原子扩展`verify:s0-s3`加入非空空间Session component suite与Web production build。
5. 完成条件：组件证据证明ready创建、starting/unavailable不创建、身份变化有序销毁、撤权清DOM/查询失效/迟到代次拒绝和资源基址绑定。

### B4恢复与Web切换

1. 将Protyle Core构造器、Transport消费和ResourcePort一次性切为S3真实Session合同，删除`App`、旧`fetchPost`、任意请求头字典、全局资源地址和旧`layout/Model`依赖。
...
| --- | --- | --- | --- |
| S0 database owner | `enterprise/packages/database/**` | contracts与API package本地源码 | database迁移与测试export定稿后在`/root/projects/mailbox.md`释放 |
| S0 contracts owner | `enterprise/packages/contracts/**` | database schema、API模块源码 | 合同字段与生成入口定稿后释放 |
| S0 API owner | `enterprise/apps/api/**` | package本地源码与只读审计 | 依赖清单提交给集成owner后释放 |
| S1-S3 Web owner | `enterprise/apps/web/src/**`中组织/空间组合根 | API、database、Kernel package源码 | 路由与Session合同case通过后释放 |
...
| S3 | 硬编码默认空间、测试Session/Factory生产装配、路由代次外的迟到Session创建 | Web owner | `verify:s0-s3` component/build |
...
- **安全**：真实空间来自数据库复合所有权和成员授权，未知Kernel路径拒绝，mTLS/JWT、头白名单和主动内容策略保护私网与应用源。
- **数据流**：路由参数只定位查询，服务端响应才产生Session；不复制正文、Kernel状态或编辑器实例到Zustand。
...
- **测试治理**：数据库、HTTP、HTTPS/WSS、Go unit、组件、production build、browser integration与E2E按最低充分层级分离，由分阶段非空`verify:s0-s3`发现。

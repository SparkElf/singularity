...
| `enterprise/apps/web/tests` | 静态shell存在，browser integration仍为空且允许无测试通过 | S1首个真实浏览器合同须替换静态shell旁路 |
...
S1在`apps/api/src`内使用三个粗粒度模块：`identity`拥有密码、KDF准入、登录限流、AuthSession、Cookie、Origin与CSRF；`spaces`拥有当前用户空间列表、两级成员授权、Kernel状态登记和启动响应；`operations`是无HTTP组合根。`AccessOperationsService`是每条运维命令的唯一事务所有者，在同一个Prisma交互事务中向`IdentityService`与`SpaceAccessService`的事务内方法传递同一`Prisma.TransactionClient`；领域Service不得开启嵌套事务。Service直接使用Prisma，不增加只做转发的Repository、DTO mapper、Facade或第二套Entity。HTTP与运维输入在边界解析一次，Service只接收规范化命令；数据库模型不流入浏览器。
...
Session是带资源生命周期的非序列化对象，由空间路由组件直接拥有，不进入Zustand。Zustand只保存跨页面且不持久化的CSRF token；TanStack Query保存服务端返回的组织、空间、角色和启动状态。

S1 React路由固定为`/login`、`/spaces`和`/organizations/:organizationId/spaces/:spaceId`。URL是当前空间唯一事实源；TanStack Query拥有授权空间摘要与启动响应，Zustand只保存不持久化的CSRF token并在退出或401时清空，组织、空间、角色和Kernel状态不复制进store。登录深链以`returnTo`保存同源站内路径，解析后只接受以`/`开头且不以`//`开头的pathname、search与hash组合，拒绝凭据、外部origin和协议相对地址；主动退出不保留`returnTo`。授权空间与启动查询覆盖全局30秒`staleTime`，固定`staleTime: 0`及`refetchOnMount: "always"`，退出或401清空整个QueryClient；独立空间查询按路由键隔离，工作台名称从当前授权空间摘要就近派生，启动响应继续只承载ID、角色与状态。
...
`deploymentHandle`固定为1至128位ASCII标识`[A-Za-z0-9][A-Za-z0-9._-]*`，不是URL；`version`为1至64位ASCII版本`[A-Za-z0-9][A-Za-z0-9.+_-]*`。每个结果含进程生成的`operationId`和唯一`outcome`；成功只取`created|updated|revoked`及上表必要ID，业务拒绝只取`already-initialized|conflict|not-found`，配置、输入、数据库或内部失败只取`failed`且不带原始原因。退出码固定为成功`0`、业务拒绝`2`、失败`1`。owner账号及其活动owner组织成员关系在S1不可停用或撤销；创建空间只保证创建时有管理员，后续空间角色仍由基础设施运维控制，不提前引入最后空间管理员计数协议。

所有冲突写入遵循`User -> Organization -> OrganizationMembership -> Space -> SpaceMembership -> AuthSession -> KernelInstance`的行锁顺序；只锁当前命令实际涉及的行，并以参数化`SELECT ... FOR UPDATE`取得锁。登录在Argon2验证后锁User、重新确认active，再在同一事务撤销请求携带的当前会话并创建新会话；停用用户和撤销其会话先锁同一User。空间成员授予/调整/撤销先锁目标User、Organization、OrganizationMembership与Space；撤销组织成员使用同序并在提交前置inactive全部SpaceMembership。这样并发登录不能越过停用，并发授予不能落在已撤销的组织成员之后；真实PostgreSQL交错测试验证最终不变量。
...
`ApiProblem.code`在既有值上增加`rate-limited`，其唯一状态为429并配`Retry-After`。Zod schema、TypeScript类型、OpenAPI 3.1 schema、API解析与浏览器解析均由`packages/contracts`同一公开常量产生或消费；空间角色继续直接消费`authorization`唯一事实源，不在contracts复制同名数组。
...
每个事实只有一个所有者。SystemInstallation只表达初始化是否完成，PostgreSQL拥有账号、会话、空间和角色，路由参数只定位查询，TanStack Query拥有浏览器服务端视图，Zustand只拥有内存CSRF，Kernel实例注册表拥有内部路由，Session拥有浏览器运行时资源，Kernel拥有内容。
...
- S1的运维写入口必须通过`IdentityService`与`SpaceAccessService`执行，会话撤销/到期、用户禁用、组织/空间停用及两级成员变化提交后，后续HTTP查询只读取新事实，不保留进程内授权缓存。S1不预建无人消费的`AccessChanged` publisher或no-op adapter；S2在`SpaceConnectionRegistry`首个生产消费者落地的同一批加入事务提交后事件，覆盖上述全部Service，并由真实WebSocket合同证明。
...
- Gateway到Kernel强制mTLS与短期服务JWT，主动内容按5.6节强制下载或安全渲染；应用源不执行上传或导出的HTML、JavaScript、SVG、XML和PDF。
...
- **Command**：运维stdin解析为判别联合命令，由单一switch分派到领域Service；不为每个低频动作创建类层级或插件系统。
...
- **Event-Driven**：S2在首个Registry消费者落地时，以事务提交后的唯一`AccessChanged`事件驱动连接关闭与到期更新；S1只固定写Service，不保留no-op publisher。
...
Nest测试应用和受控Kernel HTTPS/WSS服务都由runner hook在随机端口启动与关闭；S1的`apps/api/test/support/test-app.ts`从readiness、auth与space HTTP三个直接消费者抽取随机端口生命周期，只返回app、base URL和幂等dispose，不判断登录或授权成功。readiness与无可变进程状态的space suite可用`beforeAll/afterAll`，每个Auth case必须用`beforeEach/afterEach`创建全新Nest容器、真实内存limiter和KDF队列，确保整套运行与按名称单跑等价。临时CA、服务端证书、客户端证书和JWT密钥由后续测试生命周期创建并在失败路径删除。固定sleep禁止用于同步，HTTP等待响应，WebSocket等待open/message/close事件。
...
| React登录、深链、单/多/空空间和状态切换 | component | 真实Router、QueryClient、Zustand与组件；HTTP为外部边界 | 独立页面状态与迟到结果case；覆盖S1-P-03、07至13、16至18 |
| 退出后历史/缓存不恢复，长名称、可访问性与最窄布局成立 | browser integration | 真实Vite/Chromium；HTTP由`page.route`替换并明确标为browser integration | 1440x900、390x844、320x568三项目；back/forward、键盘名称/顺序/可见焦点、实际文本对比度AA、水平溢出/重叠及原始console/network诊断；覆盖S1-P-14/18 |
...
| `apps/api/test/support/test-app.ts` | readiness HTTP、auth HTTP、space HTTP | 随机端口启动/关闭Nest应用，返回app、基址与幂等dispose | 登录、授权或业务成功判断 | owner suite选hook；Auth每case新实例，其他无进程状态suite可共享；消费者少于两个时内联删除 |
...
| `apps/web/tests/support/browser-health.ts` | auth-navigation browser、space-states browser | 原样采集console error/warn、pageerror、requestfailed、全部HTTP响应、CORS/资源失败及关键请求pending时长并返回证据 | “意外”判定、业务允许状态、网络故障预期、页面状态与权限断言 | Playwright case生命周期；各case自行过滤并断言业务预期；消费者少于两个时内联删除 |
...
Browser integration允许拦截外部身份/空间HTTP，配置只启动Vite Web并保持`fullyParallel: true`，每个case自备业务route状态并在case内判定预期HTTP/网络失败。现有`playwright.shell.config.ts`与`tests/shell/workspace.spec.ts`的设计token、侧栏和控制台证据并入S1两个真实browser文件后删除，不保留重复runner或静态“默认空间”路径。P5真实E2E配置由Playwright `webServer`数组启动API、Web和测试Kernel，`globalSetup`执行专用测试schema迁移；在按worker创建并清理组织/空间的fixture具备两个稳定消费者前使用`fullyParallel: false`。
...
8. 完成条件：受控运维可产生真实多账号/多空间/两级成员和Kernel三态数据；浏览器只能从授权响应取得真实`spaceId`；并发撤权不变量、401/403/404/429、Cookie、CSRF、会话时效、空间状态、退出后历史清理、320px/WCAG与production build均有标准入口证据。
...
4. 实现空间化Kernel API、assets、upload、exports和只接收推送的Protyle WebSocket代理；主动内容按5.6节强制下载或安全渲染。
...
- **运行时校验**：只在HTTP、stdin、配置、Cookie/CSRF和数据库状态转换信任边界做有界校验；载荷至多16KiB且不进入编辑热路径，静态类型不能替代不可信字节与并发状态检查。用户已批准对应安全与运维产品合同。
...
- **前端系统**：radix-nova、Tailwind语义token和shadcn组件是唯一视觉事实源；TanStack Query拥有服务端视图，Zustand只拥有内存CSRF，Session不进store。

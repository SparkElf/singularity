...
S3实现审计确认该链在Runtime与Core之间仍缺身份选择：启动响应只含`spaceId`，而Core首次内容请求要求`notebookId + documentId`。本方案按[ADR-020](../adr/0020-space-content-directory-bootstrap.md)增加独立空间内容目录，使最终生产链成为`授权空间 -> 服务目录 -> 完整内容身份 -> Protyle`，且目录不读取正文、不复用或放宽内容Gateway。

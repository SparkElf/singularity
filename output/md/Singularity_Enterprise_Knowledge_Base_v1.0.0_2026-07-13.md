...
| 1.6.1 | 2026-07-22 | Codex | 完成L2统一verification，固定browser integration单worker资源合同并记录59/59与P5 12/12证据 |
| 1.7.0 | 2026-07-22 | Codex | 建立L3.0实时协作语义原型PRD、架构、ADR与可恢复实施计划，保留生产前语义门禁 |
| 1.7.1 | 2026-07-22 | Codex | 完成L3.0语义contracts、Go reducer/bridge、协议协调器、独立浏览器原型及唯一verification证据，保持生产路径隔离 |
...
| L3 | 验证并交付实时协作 | CRDT、在线状态、光标、冲突合并（先执行[L3.0 PRD](../../docs/product/l3-realtime-collaboration.md)与[架构](../../docs/architecture/l3-realtime-collaboration.md)） |
...
L2方案已完成`verification`：产品需求、架构方案、ADR-029和实施计划已冻结，评论、提及、通知、历史和文档ACL的生产实现、迁移、永久测试及唯一P5 E2E已完成，代码复评与test-governance已通过；`pnpm verify:s0-s3`通过静态、控制面、固定PostgreSQL、构建、Kernel service-auth和三视口browser integration（单worker，59 passed、0 failed、64 skipped），P5 E2E为12 passed、0 failed。L2已完成，不改变L3实时协作门禁。

L3.0阶段处于`verification`：产品、架构、ADR-030和实施计划已完成复评；语义contracts、Go reducer/bridge、双客户端协议协调器、presence/撤权生命周期和独立浏览器原型已实现。L3.0只验证普通块、结构块、块引用、嵌入、属性视图、历史、撤销、presence、四段内容身份和乱序/断线/撤权语义；不接入生产Protyle、Gateway、WebSocket、Prisma或默认实时协作入口。正式验证以`pnpm verify:l3-prototype`为唯一证据，未通过前不得进入L3.1生产候选。
...
- L2阶段状态：控制面评论、提及、通知、历史和文档ACL的生产实现、Prisma迁移、TypeScript/React永久测试及唯一P5真实链路已完成；代码复评、test-governance与统一`pnpm verify:s0-s3`已通过。browser integration配置固定`fullyParallel: false`与单worker以隔离真实Protyle DOM资源，三视口聚合为59 passed、0 failed、64 skipped，P5 E2E为12 passed、0 failed；L2已完成并进入后续L3规划。
- L3.0阶段状态：产品、架构、ADR和实施计划已完成复评并进入verification；实现、Go语义核心、协议协调器和独立浏览器原型已落盘，生产接入仍明确隔离。实时协作必须通过ADR-006规定的语义原型门禁和唯一`pnpm verify:l3-prototype`。
...
L0完成须满足第8.3节全部验收，且代码评审、测试和许可证审查通过。L1完成须满足第9.3节全部验收，且不存在已知跨空间数据泄露、未授权写入或不可恢复备份。L2完成须满足L2 PRD全部验收、ADR-029与计划门禁、统一`pnpm verify:s0-s3`及P5真实链路；不得把实时多人编辑能力混入L2。
...
11. [L3.0实时协作语义原型产品需求](../../docs/product/l3-realtime-collaboration.md)
12. [L3.0实时协作语义原型架构方案](../../docs/architecture/l3-realtime-collaboration.md)
13. [ADR-030：L3.0语义协作原型边界](../../docs/adr/0030-l3-semantic-collaboration-prototype.md)
14. [L3.0实时协作语义原型验证报告](../../docs/verification/l3-realtime-collaboration.md)

...
export const getRecentDocs = (app: App) => {
    fetchPost("/api/storage/getRecentDocs", {sortBy: "viewedAt"}, (response) => {
        let html = "";
        response.data.forEach((item: IRecentDoc, index: number) => {
            html += `<li data-index="${index}" data-node-id="${item.rootID}" data-notebook-id="${item.notebookId}" class="b3-list-item${index === 0 ? " b3-list-item--focus" : ""}">
                    ...
                    if (liElement) {
                        ...
                        if (!notebookId) {
                            console.error("[Singularity/ProtyleIdentity] recent document has no notebook", {
                                documentId: liElement.dataset.nodeId,
                            });
                            return;
                        }
                        openMobileFileById(app, notebookId, liElement.dataset.nodeId, [Constants.CB_GET_SCROLL]);
                    }
                });
            }
        });
